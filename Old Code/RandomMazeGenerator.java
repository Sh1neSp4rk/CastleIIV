...
public class RandomMazeGenerator{
	...
	boolean n,s,e,w;
	boolean notHaveStart=true;
	boolean notHaveEnd=true;
		...
		for (int panelDown = 0; panelDown<hSize;panelDown++){	
			for (int panelAcross = 0; panelAcross<hSize;panelAcross++){	
				 if (user.x == panelAcross && user.y == panelDown){
					...
					mazeBoard[panelAcross][panelDown].setIcon(new ImageIcon ("hiddenEnd.jpg"));
...
//				mazeBoard[panelDown][panelAcross].setIcon(tile[panelAcross][panelDown].image);
	...
	public void drawMaze(){
			...
			}else if (tile[user.x][user.y]==tiles[3] && user.dirEast || tile[user.x][user.y]==tiles[4] && user.dirNorth ||
			...
			}else if (tile[user.x][user.y]==tiles[3] && user.dirWest || tile[user.x][user.y]==tiles[4] && user.dirSouth ||
			...
			}else if (currentFloor==1 && tile [user.x][user.y].start==true && (tile[user.x][user.y]==tiles[11] && user.dirSouth || 
			tile[user.x][user.y]==tiles[12] && user.dirNorth || 
			tile[user.x][user.y]==tiles[13] && user.dirWest ||
			tile[user.x][user.y]==tiles[14] && user.dirEast)){
			...
			}else if (/*currentFloor==floors && */tile[user.x][user.y].end==true && (tile[user.x][user.y]==tiles[11] && user.dirSouth || 
			tile[user.x][user.y]==tiles[12] && user.dirNorth || 
			tile[user.x][user.y]==tiles[13] && user.dirWest ||
			tile[user.x][user.y]==tiles[14] && user.dirEast )){
					picLabel.setIcon(new ImageIcon("altFinish.jpg"));
